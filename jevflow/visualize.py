...
const svg=document.querySelector('svg'),zoom=document.getElementById('zoom');function resize(){{svg.style.width=({width}*zoom.value/100)+'px';svg.style.height=({height}*zoom.value/100)+'px';}}zoom.addEventListener('input',resize);resize();select({json.dumps(aliases[flow['start']])});
</script></html>'''
